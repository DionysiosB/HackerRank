...
std::vector<long> dfs(std::map<long, std::vector<long> > tree, long root, std::vector<bool> alreadyDone){

    std::vector<long> ans;
    for(int p = 0; p < tree[root].size(); p++){
        if(alreadyDone[tree[root][p]]){continue;}
        std::vector<long> subVector = dfs(tree, tree[root][p], alreadyDone);
    ...
    while(numCases--){
        ...
        */
        ...
        for(long p = 0; p < n; p++){
            ...
            std::vector<long> partial = dfs(order, p, completed);
