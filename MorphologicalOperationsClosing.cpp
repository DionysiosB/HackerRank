...
int main() {
    ...
    std::vector<std::vector<long> > f(9, std::vector<long>(10, 0));
        ...
        for(long col = 0; col < m[0].size(); col++){
            for(long dr = -1; dr <= 1; dr++){
                ...
                for(long dc = -1; dc <= 1; dc++){
                    ...
                    if(m[row + dr][col + dc] == '1'){f[row][col] = 1;}
                }
            }
        }
    }
    ...
    std::vector<std::vector<long> > g(9, std::vector<long>(10, 1));
    for(long row = 0; row < f.size(); row++){
        for(long col = 0; col < f[0].size(); col++){
            if(row == 0 || col == 0){g[row][col] = 0; continue;}
            for(long dr = -1; dr <= 1; dr++){
                if(row + dr < 0 || row + dr >= m.size()){continue;}
                for(long dc = -1; dc <= 1; dc++){
                    if(col + dc < 0 || col + dc >= m[0].size()){continue;}
                    if(!f[row + dr][col + dc]){g[row][col] = 0;}
                }
            }
        }
    }
